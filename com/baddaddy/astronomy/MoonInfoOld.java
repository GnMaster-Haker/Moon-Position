...
public class MoonInfoOld {
    ...
    public MoonInfoOld(Date date, double lat, double lon) {
        setDate(date);
    }

    private void setDate(Date javaDate) {
        double[] times = MoonTimes.getTimes(javaDate, 0.0d, 0.0d, 5);
        double Day = Astro.MJD(javaDate) - epoch;
        double M = fixAngle((elonge + fixAngle(0.9856473320990837d * Day)) - elongp);
        double Lambdasun = fixAngle(elongp + ((2.0d * Math.atan(Math.sqrt(1.0340044870138985d) * Math.tan(kepler(M, eccent) / 2.0d))) / DegRad));
        ...
        setIlluminatedFraction((1.0d - Astro.cos(MoonAgeDegrees)) / 2.0d);
        setAge(synmonth * (fixAngle(MoonAgeDegrees) / 360.0d));
        setDistance(MoonDist);
        setFraction(fixAngle(MoonAgeDegrees) / 360.0d);
        setPhase(calculatePhase(this.age, this.illuminatedFraction));
        setPerigeePercent((double) calculatePerigeePercent(this.distance));
    }
    ...
    public void setPerigeePercent(double perigeePercent) {
        this.perigeePercent = perigeePercent;
    }
